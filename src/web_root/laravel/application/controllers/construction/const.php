...
/**
 * [指定施工会社]物件管理関連コントローラー
 *
 * $Author: mizoguchi $
 * $Rev: 71 $
 * $Date: 2013-09-26 02:06:55 +0900 (2013/09/26 (木)) $
 */
class Construction_Const_Controller extends Construction_Base_Controller {
	...
	public function action_index() {
		...
		// ページ固有JS
		Asset::add('page', 'js/pages/construction/thing.js', 'common');

		return View::make('construction.thing_register', $data);
	}
	...
	public function action_confirm($page_type) {
		...
		return View::make("construction.thing_{$page_type}_check", $data);
	}
	...
	public function action_complete() {
		...
		if ( !$input ) {
			...
			return Redirect::to_action('construction.const@index');
		}
		...
		return View::make('construction.thing_register_finish');
	}
	...
	public function action_search() {
		...
		return View::make('construction.thing_search', $data);
	}
	...
	public function action_do_search() {
		...
		return View::make('construction.thing_search_result', $data);
	}
	...
	public function action_query() {
		...
		return View::make('construction.thing_query', $data);
	}
	 ...
	 * 物件照会を行ないます。
	 */
	public function action_do_query() {
		// 入力値
		$input = Input::get();
		...
		return View::make('construction.thing_query_result', $data);
	}
...
SCRIPT;
	...
	public function action_change($construction_no, $construction_eda) {
		...
		return View::make('construction.thing_change', $data);
	}
	 ...
	 * @param numeric $construction_eda 枝番
	 */
	public function action_detail($construction_no, $construction_eda) {

		$construction = Constructions::get($construction_no, $construction_eda);

		// 施工会社(必須)
		$construction_company = Companies::get($construction->construction_company)->company_name;
		$construction->construction_company = $construction_company;

		// 設計会社名
		if ( $construction->architect_company ) {
			$architect_company = Companies::get($construction->architect_company)->company_name;
			$construction->architect_company = $architect_company;
		}

		// 発注元会社名
		if ( $construction->order_company ) {
			$order_company = Companies::get($construction->order_company)->company_name;
			$construction->order_company = $order_company;
		}

		// 施工管理技術者名
		if ( $construction->engineer ) {
			$engineer = Engineers::getEngineer($construction->engineer)->name;
			$construction->engineer = $engineer;
		}

		// 設計担当者名
		if ( $construction->architect ) {
			$architect = Architects::getArchtect($construction->architect)->name;
			$construction->architect = $architect;
		}

		// 材種
		if( $construction->material_id ) {
			$materials = Materials::getAll();
			foreach ($materials as $material) {
				if ( $material->material_id == $construction->material_id ) {
					$construction->material_id = $material->material_name;
				}
			}
		}
		...
		return View::make('construction.thing_detail', $data);
	}
	...
	public function action_change_complete() {
		...
		if ( !$input ) {
			...
			return Redirect::to_action('construction.const@search');
		}
		Session::forget(KEY_FORM);
		...
		return View::make('construction.thing_change_finish', $data);
	}
