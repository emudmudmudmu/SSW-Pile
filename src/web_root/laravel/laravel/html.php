...
class HTML {
	...
	public static function script($url, $attributes = array())
	{
		...
		// 溝口追加
		// application.php にバージョン定義がある場合は
		// CSS,JSがバージョンが変わる毎にリフレッシュされるようにする
		$url = self::_add_version($url);
		
		return '<script src="'.$url.'"'.static::attributes($attributes).'></script>'.PHP_EOL;
	}
	
	private static function _add_version($url) {
		$ver = Config::get('application.version');
		if ( $ver ) {
			if ( strpos($url, '?') === FALSE ) {
				$url = "{$url}?{$ver}";
			}
			else {
				$url = "{$url}&app_version={$ver}";
			}
		}
		return $url;
	}
