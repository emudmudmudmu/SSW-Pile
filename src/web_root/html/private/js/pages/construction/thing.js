...
$(document).ready(function(e) {
	...
	$(document).on("click", "#reg_button", function (e) {
		window.location.href = base + "construction/thing_register_finish.html";
	});

	// 新規登録ボタン
	$(document).on("click", "#register_button", function (e) {
		window.location.href = base + "construction/thing_register.html";
	});
	...
	$(document).on("click", "#update_button", function (e) {
		window.location.href = base + "construction/thing_change_finish.html";
	});
});
...
function resetConstructionNo() {
	...
	q(base + "construction/get_max_seq.json", {
...
function reloadEngineers() {
	...
	q(base + "construction/get_engineers.json", {
		company_code : companyCode
	}).done(function (res) {
		if (res.status == "OK") {
...
function reloadArchitects() {
	...
	q(base + "construction/get_architects.json", {
		company_code : companyCode
	}).done(function (res) {
		if (res.status == "OK") {
			var html = '<option value=""></option>';
...
function _input_check(regist) {
	...
	if ( Bar.hasMessages() ) {
		...
		q(base + "construction/thing_check.json", data).done(function (res) {
			if ( res.status == "OK" ) {
				window.location.href = base + "construction/thing_" + (regist ? "register" : "change") + "_check.html";
			}
			else {
